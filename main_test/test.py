...
from com.sy.reco.recommendation.hybrid.content_usercf import ContentUserCF
...
class Test():
    ...
    def associationrules(self,ratingMatrix,itemNames,user_vec):
        ...
        print(recoItems)
    #结合基于user与user画像推荐
    def user_content(self,ratingMatrix,itemsFeatures,user_vec,K):
        user_cont=ContentUserCF(ratingMatrix,itemsFeatures)
        user_cont.getUsers_Ratings_Features()
        predictRating=user_cont.predictRating(user_vec,K)
        print(predictRating)
...
if __name__=='__main__':
    ...
    if recType==1:#userBased
        ...
        # 所有item的特征矩阵
        itemsFeatures = itemContentMatrix.values[:, 1:]  # itemContentMatrix第0列是item名列，其余是item的特征分布
        test.user_content(ratingMatrix,itemsFeatures,ratingMatrix[0],10)#评分数据矩阵，项目特征矩阵，测试的第0行用户,10个相似用户
